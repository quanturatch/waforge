...
# Environment:
#   WaForge_DATA_DIR   data directory to restore into (default: ./data)
#
# Stop the WaForge app before restoring. A snapshot of the current data dir is taken
...
if [ -f "$STAGE/WaForge.sqlite" ]; then
  log "Restoring data store (WaForge.sqlite)"
  cp "$STAGE/WaForge.sqlite" "$DATA_DIR/WaForge.sqlite"
fi
