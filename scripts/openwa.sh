#!/bin/bash
# WaForge Smart Orchestration Script
...
get_profiles() {
    ...
    # Dashboard is bundled into the WaForge-api image and served by NestJS on the API port —
...
cmd_start() {
    ...
    echo ""
    log_success "WaForge started successfully!"
    echo ""
...
# Stop WaForge
cmd_stop() {
    log_info "Stopping WaForge..."
    cd "$PROJECT_DIR"
    docker compose --profile full down
    log_success "WaForge stopped"
}
...
# Show status
cmd_status() {
    log_info "WaForge container status:"
    echo ""
    cd "$PROJECT_DIR"
...
# Build images
cmd_build() {
    log_info "Building WaForge images..."
    load_env
    local profiles=$(get_profiles)
    cd "$PROJECT_DIR"
...
cmd_help() {
    echo ""
    echo "WaForge Smart Orchestration Script"
    echo ""
    echo "Usage: $0 <command> [options]"
    echo ""
    echo "Commands:"
    echo "  start       Start WaForge with auto-detected profiles"
    echo "  stop        Stop all WaForge containers"
    echo "  restart     Restart WaForge"
    echo "  status      Show container status"
    echo "  logs        Show logs (default: WaForge-api)"
