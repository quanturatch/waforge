#!/bin/sh
# Smoke test: verify the built image runs its process as the WaForge user (not root).
...
IMAGE_TAG="WaForge-test-non-root:smoke"
...
# Override CMD with 'id' so docker-entrypoint.sh runs: exec gosu WaForge id
...
if echo "$USER_OUTPUT" | grep -q "WaForge"; then
  echo "PASS: process runs as WaForge (non-root)"
else
  echo "FAIL: process is not running as the WaForge user" >&2
  docker rmi "$IMAGE_TAG" > /dev/null 2>&1 || true
  exit 1
fi
