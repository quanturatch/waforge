...
#   - data store    — WaForge.sqlite (SQLite) OR a pg_dump (when DATABASE_TYPE=postgres)
#   - sessions/     — WhatsApp LocalAuth session data
#   - media/        — locally-stored media (skipped automatically when using S3)
#
# The previous runbook backed up the wrong file (WaForge.db) and omitted main.sqlite,
# so a "successful" backup silently lost every API key and all audit history.
#
# Usage:
#   ./scripts/backup.sh
# Environment:
#   WaForge_DATA_DIR   data directory (default: ./data)
...
DATA_DIR="${WaForge_DATA_DIR:-./data}"
BACKUP_DIR="${BACKUP_DIR:-./backups}"
DATABASE_TYPE="${DATABASE_TYPE:-sqlite}"
TIMESTAMP="$(date +%Y%m%d-%H%M%S)"

MAIN_DB="$DATA_DIR/main.sqlite"
DATA_DB="$DATA_DIR/WaForge.sqlite"
...
if [ "$DATABASE_TYPE" = "postgres" ]; then
  ...
  if [ -n "${DATABASE_URL:-}" ]; then
    ...
    PGPASSWORD="${DATABASE_PASSWORD:-}" pg_dump \
      ...
      -U "${DATABASE_USERNAME:-WaForge}" \
      "${DATABASE_NAME:-WaForge}" >"$STAGE/database.sql"
  fi
else
  log "Backing up data store (WaForge.sqlite)"
  backup_sqlite "$DATA_DB" "$STAGE/WaForge.sqlite"
fi
...
mkdir -p "$BACKUP_DIR"
ARCHIVE="$BACKUP_DIR/WaForge-backup-$TIMESTAMP.tar.gz"
