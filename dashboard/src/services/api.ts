// API Service Layer for WaForge Dashboard
// Centralized API client with TypeScript types

import { warnIfInsecureHttpUrl } from '../utils/urlSecurity';
import { clearApiKey, getApiKey } from '../utils/storage-keys';
...
export interface InfraStatus {
  // `builtIn` = WaForge's own bundled container is actually running and backing this service (live),
...
export const aiApi = {
  ...
  test: (text: string) =>
    request<{ reply: string; provider: string; model: string }>('/ai/test', {
      method: 'POST',
...
export const moderationApi = {
  getGroupCleanupStatus: () =>
    request<{
      ...
      dryRun: boolean;
      requireAdmin: boolean;
    }>('/moderation/group-cleanup/status'),
  testKeyword: (text: string) =>
    request<{ matched: boolean; keyword: string | null }>('/moderation/group-cleanup/test', {
      method: 'POST',
      body: JSON.stringify({ text }),
    }),
};
...
export interface CatalogPlugin {
  ...
  license?: string;
  keywords?: string[];
  minWaForgeVersion?: string;
  testedWaForgeVersion?: string;
  homepage?: string;
  download?: string;
  installed: boolean;
