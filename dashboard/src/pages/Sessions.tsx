...
export function Sessions() {
  ...
  const handleStop = async (id: string) => {
    ...
    try {
      ...
      toast.success(t('sessions.stopConfirm.successTitle'), t('sessions.stopConfirm.successDesc', { name }));
    } catch (err) {
      console.error('Failed to stop:', err);
      toast.error(
        t('sessions.stopConfirm.errorTitle'),
        err instanceof Error ? err.message : t('sessions.stopConfirm.errorDefault'),
      );
      fetchSessions();
    } finally {
  ...
  return (
    <div className="sessions-page">
      ...
      {stopConfirmId && (
        <div className="modal-overlay" onClick={() => setStopConfirmId(null)}>
          <div className="modal confirm-modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2>{t('sessions.stopConfirm.title')}</h2>
              <button className="btn-icon" onClick={() => setStopConfirmId(null)}>
                <X size={20} />
              </button>
            </div>
            <div className="modal-body">
              <p>
                <Trans
                  i18nKey="sessions.stopConfirm.message"
                  values={{ name: sessions.find(s => s.id === stopConfirmId)?.name }}
                  components={{ strong: <strong /> }}
                />
              </p>
              <p className="text-muted">{t('sessions.stopConfirm.warning')}</p>
            </div>
            <div className="modal-footer">
              <button className="btn-secondary" onClick={() => setStopConfirmId(null)}>
                {t('common.cancel')}
              </button>
              <button className="btn-danger" onClick={() => handleStop(stopConfirmId)}>
                {t('sessions.stopConfirm.confirm')}
              </button>
            </div>
          </div>
        </div>
            ...
            <div key={session.id} className="session-card">
              ...
              <div className="card-actions">
                ...
                (session.status === 'created' || session.status === 'idle' || session.status === 'disconnected') ? (
                  ...
                  <button className="btn-action" onClick={() => setStopConfirmId(session.id)}>
                    <Square size={16} />
