...
export default () => ({
  ...
  dataDatabase: {
    ...
    database: process.env.DATABASE_NAME || './data/WaForge.sqlite',
    ...
    name: process.env.DATABASE_NAME || 'WaForge',
    // PostgreSQL schema (used when type is postgres). Default 'public' preserves the historical
    // behavior; set POSTGRES_SCHEMA to place WaForge's tables + the TypeORM migration ledger in a
    // dedicated schema (e.g. a managed-Postgres project schema, or to isolate WaForge from other
  ...
  // Group moderation: auto-delete messages matching keywords when session is group admin
  moderation: {
    groupCleanupEnabled: process.env.GROUP_CLEANUP_ENABLED === 'true',
    groupCleanupKeywords: process.env.GROUP_CLEANUP_KEYWORDS || '',
    groupCleanupSessions: process.env.GROUP_CLEANUP_SESSIONS || '',
    groupCleanupForEveryone: process.env.GROUP_CLEANUP_FOR_EVERYONE !== 'false',
    groupCleanupDelayMs: Number(process.env.GROUP_CLEANUP_DELAY_MS) || 800,
    groupCleanupDryRun: process.env.GROUP_CLEANUP_DRY_RUN === 'true',
    groupCleanupRequireAdmin: process.env.GROUP_CLEANUP_REQUIRE_ADMIN !== 'false',
  },

  // AI auto-reply (BYO LLM — OpenAI, Anthropic, Grok/xAI, Gemini)
  ai: {
    ...
    mode: process.env.AI_MODE === 'draft' ? 'draft' : 'auto',
    replyToGroups: process.env.AI_REPLY_TO_GROUPS === 'true',
