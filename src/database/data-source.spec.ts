...
describe('PostgreSQL schema selection (POSTGRES_SCHEMA)', () => {
  ...
  it('passes schema through and sets extra.options search_path for a non-public schema', () => {
    const opts = buildPostgresDataSourceOptions({ POSTGRES_SCHEMA: 'WaForge' }) as PgOpts;
    expect(opts.schema).toBe('WaForge');
    expect(opts.extra?.options).toBe('-c search_path=WaForge,public');
  });
