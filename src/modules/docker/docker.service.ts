...
/**
 * The only Docker profiles WaForge manages (and may start/stop/remove). Used to bound teardown so a
...
export class DockerService implements OnModuleInit {
  ...
  /**
   * List all WaForge-related containers
   */
  async listContainers(): Promise<ContainerInfo[]> {
    ...
    try {
      ...
      return containers
        .filter(c => {
          // Filter by WaForge labels or name prefix
          const labels = c.Labels || {};
          return labels['com.WaForge.service'] || c.Names?.some(n => n.startsWith('/WaForge-'));
        })
        .map(c => ({
  ...
  /**
   * Which bundled (WaForge-managed) service containers are currently RUNNING, keyed by the
   * `com.WaForge.service` label (`database` | `cache` | `storage`). Lets the dashboard show the real
  ...
  async getRunningBuiltinServices(): Promise<{ database: boolean; cache: boolean; storage: boolean }> {
    ...
    const isRunning = (svc: string): boolean =>
      containers.some(
        c =>
          c.labels['com.WaForge.service'] === svc && c.labels['com.WaForge.builtin'] === 'true' && c.state === 'running',
      );
  ...
  async getContainerByService(service: string): Promise<Docker.Container | null> {
    ...
    try {
      const containers = await this.docker.listContainers({
        all: true,
        filters: {
          label: [`com.WaForge.service=${service}`],
        },
      });
      ...
      // string, would resolve an arbitrary container). WaForge-managed containers are `WaForge-<service>`.
      const target = `WaForge-${service}`;
  ...
  private getContainerSpec(profile: string): {
    ...
    const specs: Record<string, ReturnType<typeof this.getContainerSpec>> = {
      redis: {
        ...
        volumes: [{ name: 'WaForge_redis-data', path: '/data' }],
        healthcheck: {
        ...
        labels: {
          'com.WaForge.service': 'cache',
          'com.WaForge.builtin': 'true',
        },
      },
      postgres: {
        image: 'postgres:16-alpine',
        name: 'WaForge-postgres',
        alias: 'postgres',
        // Use hardcoded defaults for built-in container (don't inherit SQLite paths)
        env: ['POSTGRES_USER=WaForge', 'POSTGRES_PASSWORD=WaForge', 'POSTGRES_DB=WaForge'],
        volumes: [{ name: 'WaForge_postgres-data', path: '/var/lib/postgresql/data' }],
        healthcheck: {
          test: ['CMD-SHELL', 'pg_isready -U WaForge'],
          interval: 5000000000,
          timeout: 3000000000,
          retries: 5,
        },
        labels: {
          'com.WaForge.service': 'database',
          'com.WaForge.builtin': 'true',
        },
      },
      minio: {
        ...
        volumes: [{ name: 'WaForge_minio-data', path: '/data' }],
        ports: [
