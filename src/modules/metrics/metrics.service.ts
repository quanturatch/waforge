...
/**
 * Prometheus exposition for WaForge. Kept dependency-free (no prom-client) — the
...
export class MetricsService {
  ...
  async render(): Promise<string> {
    ...
    gauge('WaForge_up', 'Whether the WaForge process is up (always 1 when scraped).', 1);
    gauge('WaForge_process_uptime_seconds', 'Process uptime in seconds.', Math.round(process.uptime()));
    gauge('WaForge_process_resident_memory_bytes', 'Resident set size in bytes.', mem.rss);
    gauge('WaForge_process_heap_used_bytes', 'V8 heap used in bytes.', mem.heapUsed);

    gauge('WaForge_sessions_total', 'Total number of configured sessions.', overview.sessions.total);
    gauge('WaForge_sessions_active', 'Number of READY (active) sessions.', overview.sessions.active);

    // Per-status session counts share one metric name with a `status` label.
    lines.push('# HELP WaForge_sessions Number of sessions by status.');
    lines.push('# TYPE WaForge_sessions gauge');
    for (const [status, count] of Object.entries(overview.sessions.byStatus)) {
      lines.push(`WaForge_sessions{status="${this.escapeLabel(status)}"} ${count}`);
    }

    lines.push('# HELP WaForge_messages_total Total messages by direction.');
    lines.push('# TYPE WaForge_messages_total counter');
    lines.push(`WaForge_messages_total{direction="outgoing"} ${overview.messages.sent}`);
    lines.push(`WaForge_messages_total{direction="incoming"} ${overview.messages.received}`);

    lines.push('# HELP WaForge_messages_failed_total Total messages in FAILED state.');
    lines.push('# TYPE WaForge_messages_failed_total counter');
    lines.push(`WaForge_messages_failed_total ${overview.messages.failed}`);

    lines.push(
      '# HELP WaForge_webhook_delivery_failures_total Webhook deliveries that terminally failed (all retries exhausted) since process start.',
    );
    lines.push('# TYPE WaForge_webhook_delivery_failures_total counter');
    lines.push(`WaForge_webhook_delivery_failures_total ${getWebhookDeliveryFailuresTotal()}`);
