...
describe('MetricsService', () => {
  ...
  describe('render', () => {
    it('emits Prometheus exposition with session + message gauges', async () => {
      ...
      expect(out).toContain('WaForge_up 1');
      expect(out).toContain('WaForge_sessions_active 2');
      expect(out).toContain('WaForge_sessions_total 3');
      expect(out).toContain('WaForge_sessions{status="ready"} 2');
      expect(out).toContain('WaForge_sessions{status="failed"} 1');
      expect(out).toContain('WaForge_messages_total{direction="outgoing"} 100');
      expect(out).toContain('WaForge_messages_total{direction="incoming"} 50');
      expect(out).toContain('WaForge_messages_failed_total 3');
      // Every metric must declare HELP/TYPE before its sample.
      expect(out).toContain('# TYPE WaForge_messages_total counter');
      // Webhook terminal-failure counter is emitted with correct counter typing + current total.
      expect(out).toContain('# TYPE WaForge_webhook_delivery_failures_total counter');
      expect(out).toContain(`WaForge_webhook_delivery_failures_total ${getWebhookDeliveryFailuresTotal()}`);
      expect(out.endsWith('\n')).toBe(true);
    });
