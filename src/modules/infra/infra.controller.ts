...
interface InfraStatus {
  // `builtIn` reflects whether WaForge's own bundled container is actually running and backing this
...
interface SaveConfigDto {
  ...
  /** MCP agent tools transport (requires restart). */
  mcp?: {
    enabled?: boolean;
    readonly?: boolean;
    rateLimitMax?: number;
  ...
  /** AI auto-reply (BYO LLM key; requires restart for enable flag). */
  ai?: {
  ...
  /** Auto-delete group messages matching keywords when session is group admin. */
  groupCleanup?: {
    enabled?: boolean;
    keywords?: string;
    sessions?: string;
...
export class InfraController {
  ...
  getConfig(): SavedConfigResponse {
    ...
    return {
      ...
      mcp: {
        // Prefer live process env (what actually runs) then saved file.
        enabled: (process.env.MCP_ENABLED ?? saved.MCP_ENABLED) === 'true',
        readonly: (process.env.MCP_READONLY ?? saved.MCP_READONLY ?? 'true') !== 'false',
        rateLimitMax: Number(process.env.MCP_RATE_LIMIT_MAX || saved.MCP_RATE_LIMIT_MAX || 60) || 60,
        rateLimitWindowMs:
          Number(process.env.MCP_RATE_LIMIT_WINDOW_MS || saved.MCP_RATE_LIMIT_WINDOW_MS || 60000) || 60000,
      },
      ai: {
        autoReplyEnabled: (process.env.AI_AUTO_REPLY_ENABLED ?? saved.AI_AUTO_REPLY_ENABLED) === 'true',
        provider: process.env.AI_PROVIDER || saved.AI_PROVIDER || 'openai',
        model: process.env.AI_MODEL || saved.AI_MODEL || '',
        baseUrl: process.env.AI_BASE_URL || saved.AI_BASE_URL || '',
        systemPrompt: process.env.AI_SYSTEM_PROMPT || saved.AI_SYSTEM_PROMPT || '',
        mode: (process.env.AI_MODE || saved.AI_MODE || 'auto') === 'draft' ? 'draft' : 'auto',
        replyToGroups: (process.env.AI_REPLY_TO_GROUPS ?? saved.AI_REPLY_TO_GROUPS) === 'true',
        maxTokens: Number(process.env.AI_MAX_TOKENS || saved.AI_MAX_TOKENS || 512) || 512,
        temperature: Number(process.env.AI_TEMPERATURE || saved.AI_TEMPERATURE || 0.7) || 0.7,
        apiKeySet: Boolean(process.env.AI_API_KEY || saved.AI_API_KEY),
      },
      groupCleanup: {
        enabled: (process.env.GROUP_CLEANUP_ENABLED ?? saved.GROUP_CLEANUP_ENABLED) === 'true',
        keywords: process.env.GROUP_CLEANUP_KEYWORDS || saved.GROUP_CLEANUP_KEYWORDS || '',
        sessions: process.env.GROUP_CLEANUP_SESSIONS || saved.GROUP_CLEANUP_SESSIONS || '',
        forEveryone:
          (process.env.GROUP_CLEANUP_FOR_EVERYONE ?? saved.GROUP_CLEANUP_FOR_EVERYONE ?? 'true') !== 'false',
        dryRun: (process.env.GROUP_CLEANUP_DRY_RUN ?? saved.GROUP_CLEANUP_DRY_RUN) === 'true',
        requireAdmin:
          (process.env.GROUP_CLEANUP_REQUIRE_ADMIN ?? saved.GROUP_CLEANUP_REQUIRE_ADMIN ?? 'true') !== 'false',
        delayMs: Number(process.env.GROUP_CLEANUP_DELAY_MS || saved.GROUP_CLEANUP_DELAY_MS || 800) || 800,
      },
    };
  }
  ...
  saveConfig(@Body() config: SaveConfigDto): { message: string; saved: boolean; envPath: string; profiles: string[] } {
    try {
      ...
      if (config.database) {
        ...
        if (config.database.type === 'postgres') {
          if (config.database.builtIn) {
            ...
            updates.DATABASE_USERNAME = 'WaForge';
            updates.DATABASE_PASSWORD = 'WaForge';
            updates.DATABASE_NAME = 'WaForge';
            ...
            updates.DATABASE_NAME = config.database.database || 'WaForge';
      ...
      // MCP agent transport (Streamable HTTP at POST /mcp). Enabling requires restart so the
      // module can mount (or unmount) cleanly.
      if (config.mcp) {
        updates.MCP_ENABLED = config.mcp.enabled ? 'true' : 'false';
        updates.MCP_READONLY = config.mcp.readonly === false ? 'false' : 'true';
        if (config.mcp.rateLimitMax !== undefined) {
          updates.MCP_RATE_LIMIT_MAX = String(Math.max(1, Math.floor(config.mcp.rateLimitMax) || 60));
        }
        if (config.mcp.rateLimitWindowMs !== undefined) {
          updates.MCP_RATE_LIMIT_WINDOW_MS = String(
            Math.max(1000, Math.floor(config.mcp.rateLimitWindowMs) || 60000),
          );
        }
      }

      // AI auto-reply (BYO OpenAI / Claude / Grok / Gemini).
      if (config.ai) {
        updates.AI_AUTO_REPLY_ENABLED = config.ai.autoReplyEnabled ? 'true' : 'false';
        if (config.ai.provider) {
          const allowed = ['openai', 'anthropic', 'grok', 'gemini'];
          if (!allowed.includes(config.ai.provider)) {
            throw new BadRequestException(`Unknown AI provider: ${config.ai.provider}`);
          }
          updates.AI_PROVIDER = config.ai.provider;
        }
        setSecret('AI_API_KEY', config.ai.apiKey);
        if (config.ai.model !== undefined) updates.AI_MODEL = config.ai.model;
        if (config.ai.baseUrl !== undefined) updates.AI_BASE_URL = config.ai.baseUrl;
        if (config.ai.systemPrompt !== undefined) updates.AI_SYSTEM_PROMPT = config.ai.systemPrompt;
        if (config.ai.mode) updates.AI_MODE = config.ai.mode === 'draft' ? 'draft' : 'auto';
        if (config.ai.replyToGroups !== undefined) {
          updates.AI_REPLY_TO_GROUPS = config.ai.replyToGroups ? 'true' : 'false';
        }
        if (config.ai.maxTokens !== undefined) {
          updates.AI_MAX_TOKENS = String(Math.max(16, Math.floor(config.ai.maxTokens) || 512));
        }
        if (config.ai.temperature !== undefined) {
          const t = Number(config.ai.temperature);
          updates.AI_TEMPERATURE = String(Number.isFinite(t) ? Math.min(2, Math.max(0, t)) : 0.7);
        }
      }

      // Group keyword auto-cleanup (delete matching messages when session is group admin).
      if (config.groupCleanup) {
        updates.GROUP_CLEANUP_ENABLED = config.groupCleanup.enabled ? 'true' : 'false';
        if (config.groupCleanup.keywords !== undefined) {
          updates.GROUP_CLEANUP_KEYWORDS = config.groupCleanup.keywords;
        }
        if (config.groupCleanup.sessions !== undefined) {
          updates.GROUP_CLEANUP_SESSIONS = config.groupCleanup.sessions;
        }
        if (config.groupCleanup.forEveryone !== undefined) {
          updates.GROUP_CLEANUP_FOR_EVERYONE = config.groupCleanup.forEveryone ? 'true' : 'false';
        }
        if (config.groupCleanup.dryRun !== undefined) {
          updates.GROUP_CLEANUP_DRY_RUN = config.groupCleanup.dryRun ? 'true' : 'false';
        }
        if (config.groupCleanup.requireAdmin !== undefined) {
          updates.GROUP_CLEANUP_REQUIRE_ADMIN = config.groupCleanup.requireAdmin ? 'true' : 'false';
        }
        if (config.groupCleanup.delayMs !== undefined) {
          updates.GROUP_CLEANUP_DELAY_MS = String(
            Math.max(0, Math.min(15000, Math.floor(config.groupCleanup.delayMs) || 800)),
          );
        }
      }
  ...
  async requestRestart(@Body() body?: { profiles?: string[]; profilesToRemove?: string[] }): Promise<{
    ...
    if (this.dockerService.isDockerAvailable()) {
      ...
      // Only ever tear down WaForge-managed services. An arbitrary profile name (or the empty string)
  ...
  async exportData(): Promise<{
    ...
    // TypeORM 0.3.6's query() has no type parameter — cast the Promise<any> result.
    const sessions = (await this.dataDataSource.query('SELECT * FROM sessions')) as SessionRow[];
    const webhooks = (await this.dataDataSource.query('SELECT * FROM webhooks')) as WebhookRow[];
    ...
    try {
      messages = (await this.dataDataSource.query('SELECT * FROM messages')) as MessageRow[];
    } catch (error) {
      this.logger.debug('Messages table not available for export', { error: String(error) });
    }

    try {
      messageBatches = (await this.dataDataSource.query(
    ...
    try {
      templates = (await this.dataDataSource.query('SELECT * FROM templates')) as TemplateRow[];
    } catch (error) {
      this.logger.debug('Templates table not available for export', { error: String(error) });
    }

    try {
      baileysStoredMessages = (await this.dataDataSource.query(
        'SELECT * FROM baileys_stored_messages',
      )) as BaileysStoredMessageRow[];
    } catch (error) {
      this.logger.debug('Baileys stored messages table not available for export', { error: String(error) });
    }

    try {
      lidMappings = (await this.dataDataSource.query('SELECT * FROM lid_mappings')) as LidMappingRow[];
    } catch (error) {
    ...
    try {
      webhookDeliveryFailures = (await this.dataDataSource.query(
        'SELECT * FROM webhook_delivery_failures',
      )) as WebhookDeliveryFailureRow[];
    } catch (error) {
      this.logger.debug('webhook_delivery_failures table not available for export', { error: String(error) });
    }
    try {
      integrationDeliveryFailures = (await this.dataDataSource.query(
        'SELECT * FROM integration_delivery_failures',
      )) as IntegrationDeliveryFailureRow[];
    } catch (error) {
