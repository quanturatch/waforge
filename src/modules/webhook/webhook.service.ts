...
export class WebhookService implements OnModuleInit, OnModuleDestroy {
  ...
  async test(sessionId: string, webhookId: string): Promise<{ success: boolean; statusCode?: number; error?: string }> {
    ...
    const testPayload: WebhookPayload = {
      ...
      data: {
        message: 'This is a test webhook from WaForge',
    ...
    const headers: Record<string, string> = {
      ...
      'User-Agent': 'WaForge-Webhook/1.0.0',
      'X-WaForge-Event': 'test',
      'X-WaForge-Idempotency-Key': testPayload.idempotencyKey,
      'X-WaForge-Delivery-Id': testPayload.deliveryId,
      'X-WaForge-Retry-Count': '0',
    };

    if (webhook.secret) {
      headers['X-WaForge-Signature'] = this.generateSignature(body, webhook.secret);
    }
  ...
  async dispatch(sessionId: string, event: string, data: Record<string, unknown>): Promise<void> {
    ...
    const deliverOne = async (webhook: Webhook): Promise<void> => {
      ...
      // (receivers dedupe on the X-WaForge-Idempotency-Key header). Re-assert them onto the post-hook
      ...
      const headers: Record<string, string> = {
        ...
        'Content-Type': 'application/json',
        'User-Agent': 'WaForge-Webhook/1.0.0',
        'X-WaForge-Event': event,
        'X-WaForge-Idempotency-Key': idempotencyKey,
        'X-WaForge-Delivery-Id': deliveryId,
        'X-WaForge-Retry-Count': '0',
      };
      ...
      if (this.queueEnabled && this.webhookQueue) {
        try {
          ...
          if (webhook.secret) {
            headers['X-WaForge-Signature'] = signature;
          }
          ...
          // same X-WaForge-Idempotency-Key / X-WaForge-Delivery-Id, so a conformant receiver dedupes.
          try {
  ...
  private async deliverWebhook(
    ...
    headers['X-WaForge-Retry-Count'] = String(attempt - 1);

    // Add signature if secret is configured and not already present
    if (webhook.secret && !headers['X-WaForge-Signature']) {
      headers['X-WaForge-Signature'] = this.generateSignature(body, webhook.secret);
    }
   ...
   * X-WaForge-* header) so a webhook config cannot forge the signature/event/idempotency
  ...
  private sanitizeCustomHeaders(custom: Record<string, string> | null | undefined): Record<string, string> {
    ...
    for (const [key, value] of Object.entries(custom ?? {})) {
      if (!/^(content-type|x-WaForge-)/i.test(key)) {
        safe[key] = value;
      }
    }
    return safe;
