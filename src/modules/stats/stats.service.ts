 ...
 * Dashboard charts treat null as "normalize the id" (phone format / private contact / group).
 */
export function sanitizeChatName(chatName: string | null | undefined, chatId: string): string | null {
  const name = (chatName ?? '').trim();
  if (!name) return null;
  if (name === chatId) return null;
  const local = chatId.split('@')[0]?.split(':')[0] ?? '';
  if (local && name === local) return null;
  if (/^\d{10,}$/.test(name) && name === local.replace(/\D/g, '')) return null;
  return name;
}
  ...
  async getMessageStats(period: '24h' | '7d' | '30d'): Promise<MessageStats> {
    ...
    return {
      ...
      topChats: topChats.map(c => ({
        chatId: c.chatId,
        // Drop names that are just the raw id/digits — the dashboard normalizes those as
        // privacy LIDs / phones; a digit-only "name" made the bar chart look like garbage.
        chatName: sanitizeChatName(c.chatName, c.chatId),
