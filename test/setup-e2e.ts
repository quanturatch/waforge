...
// ./data/WaForge.sqlite. e2e creates sessions/webhooks and doesn't self-clean, so without this they
// pile up across runs. Start each run from a fresh file.
const e2eDataDb = join(tmpdir(), `WaForge-e2e-${process.pid}.sqlite`);
...
const e2eMainDb = join(tmpdir(), `WaForge-e2e-main-${process.pid}.sqlite`);
