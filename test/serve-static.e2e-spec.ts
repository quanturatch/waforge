...
const distDir = mkdtempSync(join(tmpdir(), 'WaForge-dash-'));
writeFileSync(join(distDir, 'index.html'), '<!doctype html><title>WaForge Dashboard</title>');
...
describe('Dashboard serve-static (e2e)', () => {
  ...
  it('serves the dashboard index.html at /', async () => {
    ...
    expect(res.text).toContain('WaForge Dashboard');
  });
  ...
  it('returns a JSON 404 (not the SPA) for unknown /api routes', async () => {
    ...
    expect(res.text).not.toContain('WaForge Dashboard');
  });
});
