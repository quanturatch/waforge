# Panduan Deployment Docker WaForge

Panduan ini menjelaskan cara melakukan deployment **WaForge** (WhatsApp API Gateway) menggunakan Docker dan Docker Compose.
...
WaForge menggunakan fitur **Profiles** di Docker Compose untuk mempermudah orkestrasi layanan tambahan (database, cache, dll) sesuai kebutuhan infrastruktur Anda.

### Layanan Utama (Core)
*   **`WaForge-api`**: Server backend utama REST API WaForge, sekaligus menyajikan Dashboard web (React UI) pada port yang sama.
...
git clone git@github.com:jimmimohtar/WaForge.git
cd WaForge
...
| `DATABASE_NAME` | `/app/data/WaForge.sqlite` | Lokasi database SQLite atau nama database PostgreSQL. |
...
Melihat log aktivitas sistem WaForge:
```bash
docker compose logs -f WaForge-api
```
