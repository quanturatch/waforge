...
WaForge supports multiple infrastructure configurations. Choose based on your needs:
...
# Optional: seed a known admin key. If omitted, WaForge generates a random key and writes data/.api-key.
API_MASTER_KEY=
...
**Check logs:**
```bash
docker compose logs WaForge-api --tail 100
```
...
docker run --shm-size=2gb WaForge
```
