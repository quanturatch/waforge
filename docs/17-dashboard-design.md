...
The dashboard is a web-based management interface for WaForge that lets users manage sessions, webhooks, and monitor activity without using the API directly.
...
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │  🔗 Main Webhook                                   ✅ Active   │  │
│  │  https://api.example.com/webhook/WaForge                        │  │
...
async function request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  const apiKey = sessionStorage.getItem('WaForge_api_key');
...
`localStorage` under `WaForge_theme` and `WaForge_palette`:
...
- **Palette** — one of seven accent palettes (`WaForge`, `blue`, `graphite`, `indigo`, `amber`,
...
export function useTheme() {
  const [theme, setTheme] = useState<Theme>(/* localStorage 'WaForge_theme' ?? 'system' */);
  const [palette, setPalette] = useState<ThemePalette>(/* localStorage 'WaForge_palette' ?? 'WaForge' */);

  useEffect(() => {
    const root = document.documentElement;
    if (theme === 'system') root.removeAttribute('data-theme');
    else root.setAttribute('data-theme', theme);
    localStorage.setItem('WaForge_theme', theme);
  }, [theme]);

  useEffect(() => {
    document.documentElement.setAttribute('data-palette', palette);
    localStorage.setItem('WaForge_palette', palette);
  }, [palette]);
