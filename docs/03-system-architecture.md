...
flowchart TB
    ...
    subgraph WaForge["WaForge Platform"]
...
WaForge is designed with a **Pluggable Architecture** that allows infrastructure components to be swapped without changing application code. This enables flexible deployments ranging from minimal single-session bots to larger single-node, multi-session installs.

> **Note — single-instance:** the live WhatsApp engine layer is stateful and held in-process
> (an in-memory `Map` in `SessionService`). WaForge currently runs as **one API instance per
...
WaForge does **not** use a dynamic `AdaptersModule` or string DI tokens. `AppModule`
...
> [!IMPORTANT]
> Engine abstraction is critical to mitigate **R001: WhatsApp Protocol Changes** in Risk Management. WaForge ships two production-ready engines selectable via `ENGINE_TYPE`: `whatsapp-web.js` (default, Chromium/Puppeteer-based) and `baileys` (browser-free, WebSocket/Noise protocol). With the abstraction layer, adding further engines requires no changes to application code.
...
WaForge uses the adapter pattern for infrastructure components that can be swapped per deployment needs. This allows users with limited resources to run WaForge without heavyweight external dependencies.

### Adapter Overview

```mermaid
flowchart TB
    subgraph Core["WaForge Core"]
...
WaForge supports SQLite for lightweight deployments and PostgreSQL for high-volume production.
...
if (dbType === 'postgres') {
  return {
    ...
    database: configService.get('dataDatabase.name', 'WaForge'),
...
return {
  ...
  database: configService.get('dataDatabase.database', './data/WaForge.sqlite'),
  synchronize,
...
> **Note:** WaForge does not currently apply SQLite-specific concurrency hardening. There is **no**
...
WaForge provides several deployment profiles for different needs:
...
DATABASE_NAME=./data/WaForge.sqlite
...
> WaForge runs as a single API instance per session-data volume; there is no cluster-mode flag.
...
WaForge does not auto-detect a profile at runtime; pick one by available resources and expected load:
