...
This document provides a comprehensive guide for migrating WaForge, including:
...
WaForge v0.2+ includes built-in migration API endpoints that leverage the **Dual-Database Architecture**:
...
WaForge v0.2+ supports migrating media files between storage backends:
...
Redis in WaForge is used **only for caching** with TTL-based expiration. Cache data is ephemeral and automatically regenerates from the database.
...
const config: MigrationConfig = {
  sqlitePath: process.env.SQLITE_PATH || './data/WaForge.db',
  postgresUrl: process.env.DATABASE_URL || 'postgresql://user:pass@localhost:5432/WaForge',
...
docker exec WaForge-db pg_dump -U postgres WaForge > backup.sql
...
export DATABASE_URL=postgresql://user:pass@localhost:5432/WaForge
...
ssh old-server "sqlite3 /data/WaForge.db \
    \"SELECT * FROM sessions WHERE id='${SESSION_ID}'\" \
    -csv" > session_record.csv

# Import to new database
echo "📥 Importing session record..."
ssh new-server "sqlite3 /data/WaForge.db \
...
echo "🚀 Upgrading WaForge v0.1.x → v0.2.x"
...
docker exec WaForge npm run cli -- create-api-key --name "migrated-key"
...
echo "🚀 Upgrading WaForge v0.2.x → v1.0.0"

# Pre-flight checks
CURRENT_VERSION=$(docker inspect ghcr.io/rmyndharis/WaForge --format '{{.Config.Labels.version}}' 2>/dev/null || echo "unknown")
...
docker run --rm \
  -v $(pwd)/data:/app/data \
  --env-file .env \
  ghcr.io/rmyndharis/WaForge:1.0.0 \
    ...
    - name: Configure staging webhooks
      command: |
        npm run cli -- set-webhook \
          --url https://staging-webhook.example.com/WaForge \
...
**Fix (automatic for most deployments):** WaForge ships a guard migration (`NormalizeSynchronizeUuidColumns`, ordered before the first collision) that converts the affected `uuid` columns to `varchar` on the next boot. For small-to-medium databases this is transparent — upgrade and restart.
...
DATABASE_TYPE=postgres DATABASE_HOST=... DATABASE_USERNAME=... \
  DATABASE_PASSWORD=... DATABASE_NAME=WaForge npm run migration:run
docker compose up -d
```
...
sqlite3 ./data/WaForge.db "PRAGMA integrity_check;"

# Verify auth session files
ls -la ./data/.wwebjs_auth/session-*/

# Check file permissions
stat ./data/WaForge.db
