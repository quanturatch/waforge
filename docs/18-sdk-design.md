...
WaForge ships three official, hand-written client libraries for the REST API. They are not generated from an OpenAPI spec — each is written directly against the real API surface (paths, request DTOs, response shapes) and **unit-tested with a mocked HTTP transport that asserts on the exact request path, method, and body**, so contract drift is caught at test time rather than in production.

| Language | Package | Install | Notes |
| --- | --- | --- | --- |
| JavaScript / TypeScript | [`@rmyndharis/WaForge`](https://www.npmjs.com/package/@rmyndharis/WaForge) | `npm install @rmyndharis/WaForge` | Dual ESM + CJS, bundled `.d.ts` types, Node 18+ |
| Python | [`rmyndharis-WaForge`](https://pypi.org/project/rmyndharis-WaForge/) | `pip install rmyndharis-WaForge` | Synchronous (httpx), PEP 561 typed, Python 3.9+ |
| PHP | [`rmyndharis/WaForge`](https://packagist.org/packages/rmyndharis/WaForge) | `composer require rmyndharis/WaForge` | Synchronous (Guzzle 7), PSR-4, PHP 8.1+ |

> The import names differ from the dist names where the ecosystem requires it. Python installs `rmyndharis-WaForge` but imports `WaForge`; the client class is `WaForgeClient` in JS/Python and `WaForge\Client` in PHP.

### Design Principles

- **One client, fluent resources.** A single client object (`WaForgeClient` / `WaForge\Client`) exposes every resource as a property — `client.messages.sendText(...)`, `client.sessions.start(...)`. All three SDKs expose the **same** resource surface; only the language idioms differ (camelCase methods + objects in JS/PHP, snake_case methods + dicts in Python).
...
The official JavaScript/TypeScript SDK is published as **`@rmyndharis/WaForge`**. It is a pure promise-based HTTP client: a single `WaForgeClient` exposes every API resource as a typed property. There is no event model — the SDK does not open WebSockets, emit events, or expose `client.on(...)`. To receive inbound messages and acks, configure a webhook (see the `webhooks` resource) and host your own HTTP receiver.
...
```bash
npm install @rmyndharis/WaForge
# or
yarn add @rmyndharis/WaForge
# or
pnpm add @rmyndharis/WaForge
```
...
```typescript
import { WaForgeClient } from '@rmyndharis/WaForge';

const client = new WaForgeClient({
...
async function main() {
  ...
  const result = await client.messages.sendText('my-session', {
    ...
    text: 'Hello from the WaForge SDK!',
  });
...
```javascript
const { WaForgeClient } = require('@rmyndharis/WaForge');
```

### Client Configuration

The constructor takes a single `WaForgeClientOptions` object. `baseUrl` and `apiKey` are required (the constructor throws synchronously if either is missing).

| Option | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `baseUrl` | `string` | yes | — | Base URL of the WaForge API, e.g. `http://localhost:2785`. A trailing slash is trimmed; a path prefix (e.g. `https://host/v1`) is preserved. |
...
All resources are accessed as properties on the client (`client.<resource>.<method>`). Every method returns a `Promise`. Methods marked **OPERATOR** require an API key with the `OPERATOR` role; a non-operator key receives a `403` (`WaForgeForbiddenError`).
...
On a non-2xx response the SDK throws a typed `WaForgeApiError` subclass carrying `.status` (HTTP status), `.body` (parsed JSON error envelope, or raw text), and `.errorKind` (the NestJS `error` field). All error classes extend `WaForgeError` and are exported, so they are `instanceof`-checkable. A timeout throws `WaForgeTimeoutError`, which extends `WaForgeError` directly (not `WaForgeApiError`).

| Error class | HTTP status | Meaning |
| --- | --- | --- |
| `WaForgeAuthError` | 401 | Missing or invalid API key. |
| `WaForgeForbiddenError` | 403 | The key's role is insufficient (e.g. an OPERATOR-only route). |
| `WaForgeNotFoundError` | 404 | Resource not found. |
| `WaForgeConflictError` | 409 | Conflict — typically the engine is not ready. |
| `WaForgeRateLimitError` | 429 | Rate limited. |
| `WaForgeNotImplementedError` | 501 | The active engine does not support this operation. |
| `WaForgeApiError` | any other | Generic non-2xx (the base API error, e.g. `400`; also surfaced for unfollowed 3xx redirects). |
| `WaForgeTimeoutError` | — | The request exceeded the configured timeout. |

```typescript
import {
  WaForgeClient,
  WaForgeConflictError,
  WaForgeNotFoundError,
  WaForgeRateLimitError,
  WaForgeTimeoutError,
  WaForgeApiError,
} from '@rmyndharis/WaForge';
...
try {
  ...
  if (err instanceof WaForgeConflictError) {
    // 409 — engine not ready; start the session first.
  } else if (err instanceof WaForgeNotFoundError) {
    // 404 — session/chat does not exist.
  } else if (err instanceof WaForgeRateLimitError) {
    // 429 — back off and retry.
  } else if (err instanceof WaForgeTimeoutError) {
    // request timed out.
  } else if (err instanceof WaForgeApiError) {
...
- **Redirects are never followed.** The transport uses `redirect: 'manual'`, so a `3xx` surfaces to the caller as an error (via `WaForgeApiError`) rather than being followed — this guarantees the `X-API-Key` header is never re-sent to a redirect target (potentially a different origin).
...
```bash
pip install rmyndharis-WaForge
```

```python
from WaForge import WaForgeClient
```

- **Distribution (PyPI):** `rmyndharis-WaForge`
- **Import package:** `WaForge`
- **Client class:** `WaForgeClient`
- **Python:** `>=3.9` (per `pyproject.toml`)
- **Runtime dependency:** `httpx>=0.25.0,<1.0`
- **Typed:** ships `py.typed` markers for `WaForge` and `WaForge.resources` (PEP 561)
...
result = client.messages.send_text("my-session", {
    "chatId": "628123456789@c.us",
    "text": "Hello from the WaForge Python SDK!",
})
...
`WaForgeClient` is also a context manager, so the connection pool is closed for you:

```python
with WaForgeClient(base_url="http://localhost:2785", api_key="owa_k1_…") as client:
...
| `timeout` | `float` | `30.0` | Per-request timeout in seconds. A breach raises `WaForgeTimeoutError`. |
...
Every error inherits from `WaForgeError`. A non-2xx response raises an `WaForgeApiError` (or a more specific subclass picked by status); a timeout raises `WaForgeTimeoutError`. The API-error classes carry `.status` (HTTP code), `.body` (parsed JSON or raw text), and `.error_kind` (the NestJS `error` field).

| Exception | Trigger |
| --- | --- |
| `WaForgeAuthError` | HTTP `401` — missing or invalid API key |
| `WaForgeForbiddenError` | HTTP `403` — insufficient role |
| `WaForgeNotFoundError` | HTTP `404` — resource not found |
| `WaForgeConflictError` | HTTP `409` — typically engine-not-ready |
| `WaForgeRateLimitError` | HTTP `429` — too many requests |
| `WaForgeNotImplementedError` | HTTP `501` — active engine doesn't support the operation |
| `WaForgeApiError` | any other non-2xx status (incl. unfollowed `3xx`) |
| `WaForgeTimeoutError` | request exceeded `timeout` (has a `.timeout` attribute) |
...
client = WaForgeClient(base_url="http://localhost:2785", api_key="owa_k1_…")
...
except WaForgeConflictError:
    print("Session engine not ready yet — start it first.")
except WaForgeNotFoundError:
    print("Session does not exist.")
except WaForgeRateLimitError:
    print("Rate limited — back off and retry.")
except WaForgeTimeoutError as e:
    print(f"Timed out after {e.timeout}s")
except WaForgeApiError as e:
    print(f"API error {e.status} ({e.error_kind}): {e.body}")
```

### Notable Behaviors

- **Redirects are never followed.** `follow_redirects` is forced off so the `X-API-Key` header is never re-sent to a redirect target. An unfollowed `3xx` therefore surfaces as an `WaForgeApiError` rather than a success.
...
The PHP SDK is a hand-written, synchronous client built on Guzzle 7. It mirrors the full user-facing API surface: every resource method maps to one REST endpoint, request/response payloads are plain associative arrays, and non-2xx responses are translated into a typed `WaForge*Exception` hierarchy.

### Installation

```bash
composer require rmyndharis/WaForge
```
...
- PSR-4 autoloaded under the `WaForge\` namespace (`"WaForge\\": "src/"`).
...
Missing `baseUrl` or `apiKey` throws `WaForge\Exceptions\WaForgeException` from the constructor.
...
All exceptions live in `WaForge\Exceptions` and descend from `WaForgeException` (which extends PHP's `\Exception`). Any non-2xx response is raised as an `WaForgeApiException`; the static `classify()` factory picks the most specific subclass by status code. An `WaForgeApiException` carries the HTTP status (`getStatus(): int`), the parsed error body (`getBody(): mixed`), and the NestJS `error` kind when present (`getErrorKind(): ?string`).

| Exception | Extends | Trigger |
| --- | --- | --- |
| `WaForgeException` | `\Exception` | Base for all SDK errors (also thrown for missing `baseUrl`/`apiKey`). |
| `WaForgeApiException` | `WaForgeException` | Any non-2xx (including unfollowed 3xx and other 4xx/5xx). |
| `WaForgeAuthException` | `WaForgeApiException` | `401` — missing/invalid API key. |
| `WaForgeForbiddenException` | `WaForgeApiException` | `403` — insufficient role (e.g. operator-only endpoint). |
| `WaForgeNotFoundException` | `WaForgeApiException` | `404` — resource not found. |
| `WaForgeConflictException` | `WaForgeApiException` | `409` — conflict (e.g. engine not ready). |
| `WaForgeRateLimitException` | `WaForgeApiException` | `429` — rate limited. |
| `WaForgeNotImplementedException` | `WaForgeApiException` | `501` — active engine does not support the operation. |
| `WaForgeTimeoutException` | `WaForgeException` | Request exceeded the timeout (`getTimeout(): float`). Not an API error — has no status/body. |

```php
<?php
use WaForge\Client;
use WaForge\Exceptions\WaForgeConflictException;
use WaForge\Exceptions\WaForgeRateLimitException;
use WaForge\Exceptions\WaForgeTimeoutException;
use WaForge\Exceptions\WaForgeApiException;
...
} catch (WaForgeConflictException $e) {
    // 409 — engine not ready yet
} catch (WaForgeRateLimitException $e) {
    // 429 — back off and retry yourself (no auto-retry)
} catch (WaForgeTimeoutException $e) {
    fwrite(STDERR, "timed out after {$e->getTimeout()}s\n");
} catch (WaForgeApiException $e) {
    // any other non-2xx
...
- **Redirects are never followed.** Guzzle is configured with `allow_redirects => false`, so a `3xx` surfaces as an `WaForgeApiException` rather than being followed — the `X-API-Key` header is never re-sent to a redirect target.
...
- **PSR-4 autoloading.** Everything lives under the `WaForge\` namespace mapped to `src/`; `composer require rmyndharis/WaForge` wires up the autoloader.

## 18.5 n8n Community Node

WaForge's n8n integration is **not** part of these SDK packages. It is a separate community node maintained in its own repository, which speaks the same REST + webhook contract documented in [API Specification](./06-api-specification.md):

- An **action/HTTP** path that calls the REST endpoints (e.g. `POST /api/sessions/:id/messages/send-text`) with the `X-API-Key` header.
- A **trigger** path that registers a webhook (the `webhooks` resource) and receives inbound events, verifying the `X-WaForge-Signature` HMAC.
...
| --- | --- | --- |
| JavaScript/TypeScript | npm | `@rmyndharis/WaForge` |
| Python | PyPI | `rmyndharis-WaForge` |
| PHP | Packagist | `rmyndharis/WaForge` |
