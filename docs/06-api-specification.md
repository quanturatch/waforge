...
WaForge seeds an initial admin key on first run (printed to the startup log and written to `data/.api-key`, or `/app/data/.api-key` in Docker). Use it to mint scoped, lower-privilege keys for integrations. Full key creation, listing, rotation, and revocation are documented under the auth resource in **§6.4.9 (API Keys)**.

## 6.2 Response Format

> **WaForge returns the raw handler payload directly — there is NO `{ success, data, meta }` envelope.** A resource route returns the resource object as-is; a list route returns a **bare JSON array**. Read fields directly (`response.id`, not `response.data.id`).
...
WaForge uses **two** timestamp representations — be careful which a field is:
...
```json
{ "chatId": "628123456789@c.us", "text": "Hello from WaForge!" }
```
...
```json
{ "text": "Hello from WaForge!", "recipients": ["6281234567890@c.us"], "backgroundColor": "#25D366", "font": 2 }
```
...
Two fields — `secret` and `headers` — are **write-only**: they are accepted on create/update but are **never** returned in any response (the response DTO has no `@Expose` for them, so `fromEntity` drops them). The `secret` is used to compute the `X-WaForge-Signature: sha256=<hex>` HMAC-SHA256 header on deliveries.
...
| secret | string | no | `@IsString`, `@MaxLength(255)` | HMAC-SHA256 signing key. **Write-only** — never returned. Used for `X-WaForge-Signature`. Defaults to `null`. |
| headers | Record<string,string> | no | `@IsHeaderMap()` — flat object (not array), ≤50 entries, names match `/^[A-Za-z0-9-]+$/`, values are strings ≤1024 chars with no C0 control/DEL (CR/LF injection guard). | Custom headers added to deliveries. **Write-only** — never returned. At delivery, `content-type` and `x-WaForge-*` names are stripped. Defaults to `{}`. |
...
Prometheus exposition scrape of WaForge process + session + message metrics; gated by a `METRICS_TOKEN` bearer (disabled when the token is unset).
...
```
# HELP WaForge_up 1 if the WaForge process is running
# TYPE WaForge_up gauge
WaForge_up 1
# TYPE WaForge_process_uptime_seconds gauge
WaForge_process_uptime_seconds 3600
# TYPE WaForge_process_resident_memory_bytes gauge
WaForge_process_resident_memory_bytes 187432960
# TYPE WaForge_process_heap_used_bytes gauge
WaForge_process_heap_used_bytes 64512000
# TYPE WaForge_sessions_total gauge
WaForge_sessions_total 3
# TYPE WaForge_sessions_active gauge
WaForge_sessions_active 2
# TYPE WaForge_sessions gauge
WaForge_sessions{status="ready"} 2
WaForge_sessions{status="disconnected"} 1
# TYPE WaForge_messages_total counter
WaForge_messages_total{direction="outgoing"} 1280
WaForge_messages_total{direction="incoming"} 940
# TYPE WaForge_messages_failed_total counter
WaForge_messages_failed_total 4
```
...
| `database.host` / `.port` / `.username` / `.database` | string | No | `port` is a string | External postgres connection (defaults `localhost`/`5432`/`postgres`/`WaForge`) |
...
```json
{
  "database": { "type": "postgres", "builtIn": false, "host": "db.example.com", "port": "5432", "username": "WaForge", "password": "s3cret", "database": "WaForge", "poolSize": 10, "sslEnabled": true, "sslRejectUnauthorized": false },
...
[
  {
    ...
    "author": "WaForge-plugins",
    "download": "https://github.com/WaForge-plugins/group-translate/releases/download/v1.2.0/group-translate.zip",
    "installed": true,
...
```json
{ "url": "https://github.com/WaForge-plugins/chat-flow/releases/download/v1.0.0/chat-flow.zip" }
```
...
const socket = io('ws://localhost:2785/events', {
  auth: { apiKey: process.env.WaForge_API_KEY },
});
...
`event`, `timestamp` (ISO-8601 dispatch time), `sessionId`, `idempotencyKey`, and `deliveryId` are always present; `data` holds the event-specific payload. The same values are mirrored into request headers (below). The HMAC `signature` is **not** in the body — it travels in the `X-WaForge-Signature` header.

### Event catalog

These are the events WaForge actually emits. A webhook is registered with an `events` list; an event is delivered to a webhook when its `events` array includes the event name or `"*"`.
...
**Design your handler to be idempotent**, keyed on the `X-WaForge-Idempotency-Key` header (see below). As a server-side safety net, WaForge de-duplicates inbound `message.received` before dispatch (a re-fired event for an already-persisted message is dropped), so one webhook normally sees each inbound message once — but this is best-effort defense-in-depth and does not remove the need for consumer-side idempotency.
...
```
X-WaForge-Signature: sha256=<hex>
```
...
If no `secret` is configured the `X-WaForge-Signature` header is omitted entirely.
...
| Header | Meaning |
| --- | --- |
| `X-WaForge-Event` | The event name (mirrors `event`) |
| `X-WaForge-Idempotency-Key` | Content-derived key; **stable across retries** of the same occurrence — dedupe on this |
| `X-WaForge-Delivery-Id` | A fresh `dlv_<uuid>` generated **per delivery** (differs per retry and per webhook) — for tracing, not dedup |
| `X-WaForge-Retry-Count` | Retry attempt number (`0` = first attempt) |
| `X-WaForge-Signature` | HMAC (only when a secret is set) |
