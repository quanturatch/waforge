...
WaForge uses a database to store:
...
WaForge supports two database backends that can be selected at deployment time:
...
WaForge v0.2+ implements a **dual-database architecture** that separates boot configuration from user data:
...
When using PostgreSQL Built-in mode, WaForge automatically:
...
When using PostgreSQL, WaForge can place its tables and migration ledger in a dedicated schema via the `POSTGRES_SCHEMA` environment variable:

| Setting            | Default | Description                                                                 |
| ------------------ | ------- | --------------------------------------------------------------------------- |
| `POSTGRES_SCHEMA`  | `public` | PostgreSQL schema for WaForge tables and TypeORM migration ledger          |

**Use Cases:**
- **Managed PostgreSQL:** Use your cloud provider's project schema (e.g., a schema provisioned by the provider)
- **Multi-tenant databases:** Isolate WaForge from other applications sharing the same database
- **Clean separation:** Keep WaForge's tables organized separately from other schemas

**Configuration:**
```bash
# .env or dashboard Infrastructure page
POSTGRES_SCHEMA=WaForge  # Use a dedicated schema
...
> [!NOTE]
> TypeORM's `schema` option alone does not set the session `search_path`. WaForge additionally sets `search_path=<schema>,public` via PostgreSQL's startup `options` parameter so raw, unqualified migration DDL resolves to the configured schema. The migration ledger and all tables land in the specified schema while keeping `public` accessible for `pg_catalog` and helpers.

#### Data Migration API

WaForge provides endpoints for migrating data between database types:
...
To ensure date/time values work across both SQLite and PostgreSQL, WaForge uses a `DateTransformer` that stores dates as ISO 8601 text strings:
...
> [!NOTE]
> There is **no** PostgreSQL RANGE partitioning, `create_messages_partition()` function, or `pg_cron` schedule in WaForge. `messages` is a single plain table on both backends. The `timestamp` column uses a `bigint→number` value transformer so the REST/SDK/MCP contract returns a JS number on both SQLite and PostgreSQL.
...
WaForge runs **two separate TypeORM connections**, each with its own migrations directory and CLI DataSource:
...
# Create backup
pg_dump -Fc $DB_NAME > $BACKUP_DIR/WaForge_$DATE.dump

# Compress
gzip $BACKUP_DIR/WaForge_$DATE.dump

# Upload to S3 (optional)
aws s3 cp $BACKUP_DIR/WaForge_$DATE.dump.gz s3://backups/WaForge/
