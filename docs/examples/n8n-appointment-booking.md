...
This example shows how to use WaForge and n8n to collect appointment requests over WhatsApp, check availability in an external system, and send a confirmation or alternative time slots.
...
[Availability Source]
      │
      ├── available → [Create Booking] → [WaForge: Send Text confirmation]
      │
      └── unavailable → [WaForge: Send Text with alternative slots]
```

## Trigger

Use the **WaForge Trigger** node.

| Field | Value |
| ----- | ----- |
| Event | `message.received` |
| Session | Your connected WaForge session |
...
When the requested slot is available, create the booking in your scheduling source and send a confirmation with the **WaForge: Send Text** node.
...
- WaForge Trigger: receives `message.received` events.
...
- WaForge Send Text: sends confirmation, alternatives, or clarification.
