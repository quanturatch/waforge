...
RUN groupadd -r WaForge && useradd -r -g WaForge WaForge
RUN chown -R WaForge:WaForge /app /opt/puppeteer
USER WaForge
...
services:
  app:
    ...
    environment:
      ...
      - DATABASE_URL=postgresql://WaForge:WaForge@postgres:5432/WaForge
...
services:
  app:
    image: ghcr.io/rmyndharis/WaForge:latest
    deploy:
      replicas: 1
      resources:
        limits:
...
> [!IMPORTANT]
> **Keep `replicas: 1`.** WaForge is a single-process application: live engine state lives in an
...
> **Design sketch, not a supported topology.** WaForge is single-process with in-memory engine state,
> so the multi-`WaForge` fan-out below would corrupt WhatsApp auth across replicas. It is retained only
...
DATABASE_SQLITE_PATH=./data/WaForge.db

# Option 2: PostgreSQL (for production)
# DATABASE_TYPE=postgres
# DATABASE_URL=postgresql://user:pass@localhost:5432/WaForge
...
# Option 2: S3
# STORAGE_TYPE=s3
# STORAGE_S3_BUCKET=WaForge-media
...
These rules use the metric names WaForge actually exports (`WaForge_*`). The memory rule below uses a
...
groups:
  - name: WaForge-alerts
    rules:
      # Service Down — WaForge_up disappears (or the scrape fails)
      - alert: ServiceDown
        expr: up{job="WaForge"} == 0 or absent(WaForge_up)
        for: 1m
        labels:
          severity: critical
        annotations:
          summary: "WaForge service is down"
          description: "The WaForge application is not responding"

      # Session(s) disconnected
      - alert: SessionDisconnected
        expr: WaForge_sessions{status="disconnected"} > 0
        for: 2m
        labels:
          severity: warning
        annotations:
      ...
      - alert: FailedMessagesRising
        expr: rate(WaForge_messages_failed_total[5m]) > 0
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "Messages are failing"
          description: "WaForge_messages_failed_total is increasing over the last 5 minutes"

      # Process memory growth (app-exported RSS; ~2GB example threshold)
      - alert: HighProcessMemory
        expr: WaForge_process_resident_memory_bytes > 2e9
        for: 10m
        labels:
          severity: warning
        annotations:
          summary: "High WaForge process memory"
          description: "RSS is {{ $value | humanize1024 }}B"

      # Host memory pressure — EXTERNAL (node-exporter), not exported by WaForge
...
receivers:
  - name: 'slack-notifications'
    slack_configs:
      - channel: '#WaForge-alerts'
        send_resolved: true

  - name: 'slack-critical'
    slack_configs:
      - channel: '#WaForge-critical'
        send_resolved: true
...
The metrics surface is small, so WaForge emits Prometheus text exposition format (v0.0.4) **by hand** —
...
export class MetricsService {
  ...
  async render(): Promise<string> {
    ...
    gauge('WaForge_up', '...', 1);
    gauge('WaForge_process_uptime_seconds', '...', Math.round(process.uptime()));
    gauge('WaForge_process_resident_memory_bytes', '...', mem.rss);
    gauge('WaForge_process_heap_used_bytes', '...', mem.heapUsed);
    gauge('WaForge_sessions_total', '...', overview.sessions.total);
    gauge('WaForge_sessions_active', '...', overview.sessions.active);
    // WaForge_sessions{status="..."} — one line per status
    // WaForge_messages_total{direction="outgoing"|"incoming"}
    // WaForge_messages_failed_total
    return lines.join('\n') + '\n';
  }
}
```
...
|--------|------|--------|---------|
| `WaForge_up` | gauge | — | Always `1` when scraped |
| `WaForge_process_uptime_seconds` | gauge | — | Process uptime |
| `WaForge_process_resident_memory_bytes` | gauge | — | RSS |
| `WaForge_process_heap_used_bytes` | gauge | — | V8 heap used |
| `WaForge_sessions_total` | gauge | — | Configured sessions |
| `WaForge_sessions_active` | gauge | — | READY (active) sessions |
| `WaForge_sessions` | gauge | `status` | Session count per status |
| `WaForge_messages_total` | counter | `direction` (`incoming`/`outgoing`) | Messages by direction |
| `WaForge_messages_failed_total` | counter | — | Messages in FAILED state |

### Grafana Dashboard Definition

```json
// monitoring/grafana/dashboards/WaForge.json — panels use the WaForge_* metrics WaForge exports
{
  "title": "WaForge Dashboard",
  "uid": "WaForge-main",
  "panels": [
    {
    ...
    },
    {
      ...
      "targets": [
        { "expr": "increase(WaForge_messages_total{direction=\"outgoing\"}[24h])" }
      ]
    },
    {
      ...
      "targets": [
        { "expr": "WaForge_messages_failed_total" }
      ]
    },
    {
      ...
      "targets": [
        { "expr": "WaForge_sessions", "legendFormat": "{{status}}" }
      ]
    },
    {
      ...
      "targets": [
        { "expr": "rate(WaForge_messages_total[5m])", "legendFormat": "{{direction}}" }
      ]
    },
    {
      ...
      "targets": [
        { "expr": "WaForge_process_resident_memory_bytes / 1024 / 1024", "legendFormat": "RSS (MB)" },
        { "expr": "WaForge_process_heap_used_bytes / 1024 / 1024", "legendFormat": "Heap used (MB)" }
      ]
    },
    {
      "title": "Uptime",
      ...
      "targets": [
        { "expr": "WaForge_process_uptime_seconds" }
      ]
    }
  ]
}
...
These are the metrics WaForge actually exports at `GET /api/metrics`:

| Category | Metric | Description | Alert Idea |
|----------|--------|-------------|------------|
| **Liveness** | `WaForge_up` | Always `1` when scraped (absence/scrape-failure = down) | Target down |
| **Sessions** | `WaForge_sessions_total` | Configured sessions | Near your expected session count |
| **Sessions** | `WaForge_sessions_active` | READY (active) sessions | Drops below expected |
| **Sessions** | `WaForge_sessions{status="..."}` | Per-status counts (e.g. `disconnected`, `failed`) | `disconnected`/`failed` > 0 |
| **Messages** | `WaForge_messages_total{direction="outgoing"}` | Outgoing messages | Sudden drop |
| **Messages** | `WaForge_messages_total{direction="incoming"}` | Incoming messages | Sudden drop |
| **Messages** | `WaForge_messages_failed_total` | Messages in FAILED state | Rising rate |
| **System** | `WaForge_process_resident_memory_bytes` | RSS | Growth / near limit |
| **System** | `WaForge_process_heap_used_bytes` | V8 heap used | Growth |
| **System** | `WaForge_process_uptime_seconds` | Process uptime | Frequent restarts (resets) |

> WaForge does **not** expose request-rate, latency-histogram, webhook, queue, or Node default
...
# Download from S3
aws s3 cp s3://WaForge-backups/database/db_$BACKUP_DATE.dump.gz /tmp/
aws s3 cp s3://WaForge-backups/sessions/sessions_$BACKUP_DATE.tar.gz /tmp/
...
WaForge scales **vertically** — add CPU/RAM to a single instance. The table below is **unbenchmarked
...
**Not currently supported.** WaForge is a single-process application with in-memory engine state, so
