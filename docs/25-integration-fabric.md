...
The overriding goal is to preserve the untrusted-worker safety invariants *by construction*. WaForge
...
**~90% a faithful clone of seams WaForge already ships**:
...
flowchart LR
    ...
    subgraph Core["WaForge host (core)"]
...
- **Authentication inversion.** A provider webhook cannot carry an WaForge API key, so ingress is public to
