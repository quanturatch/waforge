...
This document contains Standard Operating Procedures (SOP) for WaForge operations, including incident response, maintenance procedures, and troubleshooting guides.
...
docker compose logs --tail=100 WaForge
...
# A. Container crashed
docker compose logs WaForge 2>&1 | grep -i "error\|fatal\|crash"

# B. Out of memory
docker compose logs WaForge 2>&1 | grep -i "oom\|memory"

# C. Database connection
docker compose logs WaForge 2>&1 | grep -i "database\|connection refused"
...
docker compose logs WaForge 2>&1 | grep -i "{sessionId}" | tail -20
...
docker stats --no-stream WaForge
free -m
...
curl -H "Authorization: Bearer $METRICS_TOKEN" \
  ...
  | grep -E "WaForge_process_resident_memory_bytes|WaForge_process_heap_used_bytes"

# 3. Check for memory leaks
docker compose logs WaForge 2>&1 | grep -i "heap\|memory\|gc"
...
docker compose logs WaForge 2>&1 | grep -i "webhook" | tail -20

# 3. Identify failure reason:
# A. Endpoint not responding
curl -v https://your-webhook-endpoint.com/WaForge

# B. SSL certificate issues
curl -v --insecure https://your-webhook-endpoint.com/WaForge

# C. Timeout
curl -v --max-time 30 https://your-webhook-endpoint.com/WaForge

# D. Authentication failed
curl -v -H "Authorization: Bearer token" \
  https://your-webhook-endpoint.com/WaForge
...
docker compose logs WaForge 2>&1 | grep -i "webhook" | tail -5
```
...
# Verify backup
ls -la /backups/WaForge/$(date +%Y%m%d)/
...
# 2. Create backup
./scripts/backup.sh
BACKUP_DIR="/backups/WaForge/$(date +%Y%m%d-%H%M%S)"

# 3. Export current state
docker compose exec WaForge npm run export -- --output /tmp/export.json
docker cp WaForge:/tmp/export.json $BACKUP_DIR/

# 4. Stop services
docker compose down

# 5. Update version in docker-compose.yml
# Change: image: ghcr.io/rmyndharis/WaForge:0.1.0
# To:     image: ghcr.io/rmyndharis/WaForge:0.7.3
...
docker compose run --rm WaForge npm run migration:run:prod
...
#   - WaForge.sqlite — user data                      (or a pg_dump when DATABASE_TYPE=postgres)
...
WaForge_DATA_DIR=/srv/WaForge/data \
  BACKUP_DIR=/backups/WaForge \
  DATABASE_TYPE=postgres DATABASE_URL=postgres://user:pass@host:5432/WaForge \
  ./scripts/backup.sh
```

> The data directory is a Docker **named volume** (`WaForge-data`) in the production
> compose. Run the script where that volume is mounted — e.g. point `WaForge_DATA_DIR`
...
tar -tzf ./backups/WaForge-backup-*.tar.gz
```
...
#    (operates on the data dir, default ./data; override with WaForge_DATA_DIR)
./scripts/restore.sh ./backups/WaForge-backup-<timestamp>.tar.gz
